...
const app = new Hono()
  ...
  .get(
    ...
    async (c) => {
      ...
      const { workspaceId, dueDate, projectId, assigneeId, status, search } =
        c.req.valid("query");
  ...
  .post(
    ...
    async (c) => {
      ...
      const higestPositionTask = await databases.listDocuments(
        DATABASE_ID,
        TASKS_ID,
        [
      ...
      const newPosition =
        higestPositionTask.documents.length > 0
          ? higestPositionTask.documents[0].position + 1000
          : 1000;
